...
resource "terraform_data" "kb_ingestion" {
  ...
  provisioner "local-exec" {
    when       = create
    on_failure = fail
    command    = "aws --profile ${var.profile} bedrock-agent start-ingestion-job --knowledge-base-id ${self.triggers_replace.kb_id} --data-source-id ${self.triggers_replace.kb_ds_id}"
  }
}
